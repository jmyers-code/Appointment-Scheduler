        ...
        if (calendarEl != null) {
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: function (fetchInfo, successCallback, failureCallback) {
                    $.ajax({
                        url: routeUrl + '/api/Appointment/GetCalendarData?nurseId=' + $("#nurseId").val(),
                        type: 'GET',
                        dataType: 'JSON',
                            ...
                            if (response.status === 1) {
                                $.each(response.dataenum, function (i, data) {
                                    events.push({
                                        ...
                                        backgroundColor: data.isNurseApproved ? "#28a745" : "#dc3545",
                                        textColor: "white",
                                        id: data.id
                                    });
                                })
...
function onShowModal(obj, isEventDetail) {
    if (isEventDetail != null) {
        ...
        $("#nurseId").val(obj.nurseId);
        $("#patientId").val(obj.patientId);
        $("#id").val(obj.id);
        $("#lblNurseName").html(obj.nurseName);
        $("#lblPatientName").html(obj.patientName);

        if (obj.isNurseApproved) {
