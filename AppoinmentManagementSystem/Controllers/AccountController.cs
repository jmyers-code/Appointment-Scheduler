...
namespace AppointmentManagementSystem.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            ...
            {
                var result = await _signInManager.PasswordSignInAsync(model.LoginCode, model.Password, model.RememberMe, false);
                if (result.Succeeded)
                {
                    var user = await _userManager.FindByNameAsync(model.LoginCode);
